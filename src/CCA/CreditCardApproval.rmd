...
Luego de unir ambos datasets observamos que la cantidad de registros de clientes con información histórica ha disminuido. Esto se debe a que no todos los clientes registrados poseen un historial crediticio. Para los fines de este análisis, aceptaremos cómo válida esta reducción.
 
 ***ToDo*** se perdieron registros! quedó mucho menor el set de datos, hay que mencionarlo
...
#ggplot(dfCreditCard) + 
#  geom_density(aes(x=CNT_FAM_MEMBERS)) + scale_x_continuous(labels=number)

#cor(dfCreditCard$LAST, dfCreditCard$DAYS_BIRTH)



#set.seed(232)
#seleccion <- runif(dim(dfCreditCard)[1])
#dftrain <- select(dfCreditCard, STATUS, Approve)[seleccion<0.8,]
#summary(dftrain)
#dftest <- select(dfCreditCard, salesLast3Mon, salesThisMon)[seleccion >= 0.8,] 
...
ggplot(dfCreditCard) + 
  geom_histogram(aes(x=CNT_FAM_MEMBERS), binwidth=1, fill="steel blue")

ggplot(dfCreditCard) + 
  geom_bar(aes(x=CNT_FAM_MEMBERS), fill="steel blue")

ggplot(dfCreditCard) + 
  geom_boxplot(aes(x=STATUS, y=CNT_FAM_MEMBERS))

ggplot(dfCreditCard) + 
  geom_boxplot(aes(x=Approve, y=CNT_FAM_MEMBERS))
...
table(dfCreditCard$STATUS)
table(dfCreditCard$LAST)
table(dfCreditCard$OCCUPATION_TYPE)
table(dfCreditCard$Approve)
#chart.Correlation(dfCreditCard, histogram = F, pch = 19)


#plot(dfCreditCard)

#status <- as.numeric(dfCreditCard$STATUS)
#status <- as.numeric(status)

#dfCreditCard$STATUS <- status
