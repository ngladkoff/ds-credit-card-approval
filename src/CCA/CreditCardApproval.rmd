...
Vamos a probar entrenar un nuevo modelo sin estas variables.

### 2.6 - Probamos re-entrenar el modelo

```{r}
predictores <- c("AGE", "CNT_FAM_MEMBERS", "FLAG_WORK_PHONE", "FLAG_EMAIL",  "NAME_FAMILY_STATUS", "OCCUPATION_TYPE")
formula <- paste(objetivo, paste(predictores, collapse = " + "), sep=" ~ ")
modeloLR <- glm(formula, data=dfTrainLR, family=binomial(link="logit"))
```
