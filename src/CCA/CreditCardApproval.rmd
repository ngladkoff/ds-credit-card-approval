...
dfTrainTrue <- filter(dfTrainLR, dfTrainLR$Approve == 1)
dfTrainFalse <- filter(dfTrainLR, dfTrainLR$Approve == 0)

trainFalseIdx <- sample(nrow(dfTrainFalse), size=3000, replace = TRUE)
dfTrainFalse <- dfTrainFalse[trainFalseIdx,]

# Probamos sin hacer este undersampling y también da mal
trainTrueIdx <- sample(nrow(dfTrainTrue), size=5000, replace = FALSE)
dfTrainTrue <- dfTrainTrue[trainTrueIdx,]


dfTrainLR <- rbind(dfTrainTrue, dfTrainFalse)
