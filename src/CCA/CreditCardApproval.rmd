...
```{r}
predictores <- c("AGE", "CNT_FAM_MEMBERS", "FLAG_WORK_PHONE", "FLAG_EMAIL", "NAME_EDUCATION_TYPE", "NAME_FAMILY_STATUS", "NAME_HOUSING_TYPE", "OCCUPATION_TYPE")
print(predictores)
```
...
```{r}
predictores <- c("FLAG_WORK_PHONE", "FLAG_EMAIL", "OCCUPATION_TYPE")
formula <- paste(objetivo, paste(predictores, collapse = " + "), sep=" ~ ")
modeloLR <- glm(formula, data=dfTrainLR, family=binomial(link="logit"))
```
...
# Base rate of disease in the training data
# proporción de enfermos que hay en los datos de entrenamiento
pnull <- mean(dfTrainLR$Approve)
pnull
